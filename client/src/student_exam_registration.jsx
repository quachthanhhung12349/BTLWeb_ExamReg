...
const RegistrationPage = ({ onLogout }) => {
  ...
  const handleRegister = (id) => {
    setSubjects(prev => prev.map(s => s.id === id ? { ...s, registered: true } : s));
  };

  const handleUnregister = (id) => {
    setSubjects(prev => prev.map(s => s.id === id ? { ...s, registered: false } : s));
  };
    ...
    <div className="d-flex vh-100 bg-light">
      ...
      <main className="flex-grow-1 p-4 overflow-auto">
        ...
        <div className="card shadow-sm border-0 mb-4 rounded-3 overflow-hidden">
                ...
                {filtered.map((s, index) => (
                  <tr key={s.id}>
                    ...
                    <td className="p-3 text-end pe-4">
                      {!s.registered ? (
                        <button className="btn btn-primary btn-sm px-3 fw-bold" onClick={() => handleRegister(s.id)} style={{ backgroundColor: '#2563eb' }}>Đăng ký</button>
                      ) : (
                        <button className="btn btn-outline-danger btn-sm px-3 fw-bold" onClick={() => handleUnregister(s.id)}>Hủy</button>
                      )}
                    </td>
                  </tr>
                ))}
          ...
          {/* PHẦN FOOTER MỚI THÊM VÀO GIỐNG ẢNH MẪU */}
          <div className="card-footer bg-white border-top p-3 d-flex justify-content-between align-items-center">
            <span className="text-muted" style={{ fontSize: '0.9rem' }}>
                Hiển thị 1-10 trên 1234
            </span>
            <div className="d-flex gap-2">
              <button className="btn btn-outline-secondary px-3 py-1 bg-white text-dark shadow-sm border-light-subtle rounded-2">
                Trước
              </button>
              <button className="btn btn-outline-secondary px-3 py-1 bg-white text-dark shadow-sm border-light-subtle rounded-2">
                Sau
